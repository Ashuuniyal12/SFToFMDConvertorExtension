            ...
            <div className="flex flex-col w-fit min-w-full">
                {sortedFields.map((field, index) => (
                    <React.Fragment key={`${field.name}-${index}`}>
                        <div
                            className={`grid items-center border-b border-border dark:border-border-dark last:border-b-0 hover:bg-[#A100FF0a] transition-colors duration-100 ${field.calculated ? 'bg-purple-50/50 dark:bg-purple-900/10' : ''}`}
                            style={{ gridTemplateColumns: gridTemplate }}
                        >
